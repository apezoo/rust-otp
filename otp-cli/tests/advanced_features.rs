...
fn test_automatic_pad_selection_for_encryption() {
    ...
    let encrypt_output = Command::cargo_bin("otp-cli").unwrap()
        .arg("--vault").arg(&vault_path)
        .arg("encrypt")
        .arg(&input_path)
    ...
    let _stdout = String::from_utf8(encrypt_output.stdout).unwrap();
...
fn test_pad_is_moved_when_fully_consumed() {
    ...
    let available_pad_path = vault_path.join("pads/available").join(format!("{}.pad", pad_id));
    assert!(available_pad_path.exists());

    // 3. Create another large file that will consume the rest of the pad
    let second_large_chunk_size = 1 * 1024 * 512; // 0.5MB
    ...
    fs::write(&second_large_input_path, &second_large_input_content).unwrap();
    ...
    let used_pad_path = vault_path.join("pads/used").join(format!("{}.pad", pad_id));
